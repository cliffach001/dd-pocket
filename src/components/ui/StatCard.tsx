...
const variantStyles = {
  blue: { bg: "bg-blue-50 text-blue-600", iconBg: "bg-blue-100" },
  green: { bg: "bg-emerald-50 text-emerald-600", iconBg: "bg-emerald-100" },
  yellow: { bg: "bg-amber-50 text-amber-600", iconBg: "bg-amber-100" },
  red: { bg: "bg-rose-50 text-rose-600", iconBg: "bg-rose-100" },
};

export default function StatCard({ icon: Icon, label, value, subtext, variant = "blue", className = "", href }: Props) {
  const styles = variantStyles[variant];
  const card = (
    <div className={`bg-white rounded-xl p-4 sm:p-5 lg:p-6 shadow-sm border border-gray-100 hover:shadow-md transition-all duration-300 hover:-translate-y-0.5 ${href ? "cursor-pointer" : ""} ${className}`}>
